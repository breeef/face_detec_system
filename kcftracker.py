...
def rearrange(img):
	...
	#print(xh,yh,img.shape[1],img.shape[0])
	...
	def update(self, image):
		...
		if(self.scale_step != 1):
			...
			if(self.scale_weight*new_peak_value1 > peak_value and new_peak_value1>new_peak_value2):
				...
				self._roi[2] *= self.scale_step
				self._roi[3] *= self.scale_step
		self.avg_peak_value.append(peak_value)
		if self.avg_peak_value:
			if peak_value > 0.8*sum(self.avg_peak_value)/len(self.avg_peak_value):
