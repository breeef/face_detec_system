...
def func1(dx, dy, boundary_x, boundary_y, height, width, numChannels):
    ...
    for j in range(1, height-1):
        for i in range(1, width-1):
            ...
            alfa[j, i, 0] = int(maxi % NUM_SECTOR)
            alfa[j, i, 1] = int(maxi)
...
def func2(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, stringSize):
    size=int(sizeX*sizeY*p)
    mapp = np.zeros((size), np.float32)
...
def func3(partOfNorm, mappmap, sizeX, sizeY, p, xp, pp):
	...
	size = int(sizeX * sizeY * pp)
	newData = np.zeros((size), np.float32)
...
def getFeatureMaps(image, k, mapp):
	...
	a_x = np.concatenate((int(k//2)- np.arange(int(k//2)) - 0.5, np.arange(int(k//2),k) - int(k//2) + 0.5)).astype(np.float32)
	b_x = np.concatenate((int(k//2) + np.arange(int(k//2)) + 0.5, -np.arange(int(k//2),k) + int(k//2) - 0.5 + k)).astype(np.float32)
